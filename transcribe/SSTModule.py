...
class STTModule:
    def __init__(self, model_size=None, device=None, lang="en"):
        ...
        # Pull from environment variables with fallback defaults
        resolved_size = model_size or os.getenv("WHISPER_MODEL_SIZE", "small").strip()
        resolved_device = device or os.getenv("WHISPER_DEVICE", "cpu").strip()

        # Match compute type based on device to prevent crashes
        compute_type = "float16" if resolved_device == "cuda" else "int8"

        print(f"📦 Loading Faster-Whisper model: [{resolved_size}] on [{resolved_device}] with [{compute_type}]")
